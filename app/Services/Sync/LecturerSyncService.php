...
class LecturerSyncService extends BaseSyncService
{
    public function syncDosen(int $offset = 0, int $limit = 500, ?string $syncSince = null): array
    {
        ...
        if (!empty($data)) {
            $records = $this->mapDosenData($data);
            $synced = $this->upsertDosen($records, $errors);
        }

        return $this->buildSyncResponse($batchCount, $synced, $errors, $totalAll, $offset, $limit);
    }

    private function mapDosenData(array $data): array
    {
        $records = [];
    ...
    public function syncAjarDosen(int $offset = 0, int $limit = 500, ?string $idSemester = null, ?string $syncSince = null): array
    {
        ...
        if (!empty($data)) {
            $records = $this->mapAjarDosenData($data, $idSemester);
            if (!empty($records)) {
    ...
    private function mapAjarDosenData(array $data, ?string $idSemester): array
    {
        $records = [];
        foreach ($data as $item) {
    ...
    private function buildSyncResponse(int $batchCount, int $synced, array $errors, int $totalAll, int $offset, int $limit): array
    {
