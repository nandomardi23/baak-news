...
class AcademicSyncService extends BaseSyncService
{
    ...
    public function syncDosenPengajar(int $offset = 0, int $limit = 2000, ?string $syncSince = null): array
    {
        ...
        if (!empty($data)) {
            ...
            if (!empty($kelasDosenUpdates)) {
                $this->updateLegacyDosenColumn($kelasDosenUpdates);
            }
        }
    ...
    public function syncKrs(int $offset = 0, int $limit = 500, ?string $idSemester = null, ?string $syncSince = null): array
    {
        ...
        */
        ...
        if (!empty($data)) {
            ...
            $mahasiswaMap = $this->autoCreateMissingMahasiswa($idRegMahasiswas, $mahasiswaMap, $data);

            $matkulMap = \App\Models\MataKuliah::whereIn('id_matkul', $idMatkuls)
                ->pluck('id', 'id_matkul');

            $matkulMap = $this->autoCreateMissingMataKuliah($idMatkuls, $matkulMap, $data);

            $semesterId = \App\Models\TahunAkademik::where('id_semester', $idSemester)->value('id');

            $this->upsertKrsHeaders($data, $mahasiswaMap, $idSemester, $semesterId, $errors);
    ...
    public function syncAktivitas(int $offset = 0, int $limit = 1000, ?string $idSemester = null, ?string $syncSince = null): array
    {
        ...
        if (!empty($data)) {
            $synced = $this->upsertAktivitasKuliahRecords($data, $errors);
    ...
    public function syncBimbinganMahasiswa(int $offset = 0, int $limit = 500, ?string $syncSince = null): array
    {
        ...
        if (!empty($data)) {
            ...
            if (!empty($records)) {
                try {
                    $this->batchUpsert(BimbinganMahasiswa::class, $records, ['id_aktivitas_mahasiswa', 'id_dosen'], ['id_bimbingan_mahasiswa', 'pembimbing_ke', 'id_kategori_kegiatan', 'updated_at']);
                    $synced = count($records);
                } catch (\Exception $e) {
    ...
    public function syncAktivitasMahasiswa(int $offset = 0, int $limit = 500, ?string $idSemester = null, ?string $syncSince = null): array
    {
        ...
        if (!empty($data)) {
            ...
            if (!empty($records)) {
                try {
                    $this->batchUpsert(AktivitasMahasiswa::class, $records, ['id_aktivitas'], ['id_jenis_aktivitas', 'nama_jenis_aktivitas', 'id_prodi', 'id_semester', 'judul', 'keterangan', 'lokasi', 'sk_tugas', 'tanggal_sk_tugas', 'updated_at']);
                    $synced = count($records);
                } catch (\Exception $e) {
    ...
    {
        return $this->processAllSemestersBulkSync('syncKrs', $offset, $limit, $syncSince);
    }
    ...
    {
        return $this->processAllSemestersBulkSync('syncNilai', $offset, $limit, $syncSince);
    }
    ...
    public function syncKrsMahasiswa(string $idRegistrasi): array
    {
        ...
        foreach ($grouped as $idSemester => $items) {
            try {
                ...
                $this->processKrsMahasiswaDetails($krs, $items);
    ...
    private function autoCreateMissingMahasiswa(array $idRegMahasiswas, \Illuminate\Support\Collection $mahasiswaMap, array $data): \Illuminate\Support\Collection
    {
                    ...
                    if ($mhsData) {
                        ...
                        if (!empty($mhsData['tanggal_lahir'])) {
                            try {
                                ...
                                try {
                                    $tanggalLahir = \Carbon\Carbon::parse($mhsData['tanggal_lahir'])->format('Y-m-d');
                                } catch (\Exception $e2) {
                                }
                            }
                        }
    ...
    private function autoCreateMissingMataKuliah(array $idMatkuls, \Illuminate\Support\Collection $matkulMap, array $data): \Illuminate\Support\Collection
    {
        $missingMatkuls = [];
        foreach ($data as $item) {
        ...
        if (!empty($missingMatkuls)) {
            ...
            return \App\Models\MataKuliah::whereIn('id_matkul', $idMatkuls)->pluck('id', 'id_matkul');
        }
        return $matkulMap;
    }

    private function upsertKrsHeaders(array $data, \Illuminate\Support\Collection $mahasiswaMap, string $idSemester, ?int $semesterId, array &$errors): void
    {
        $krsRecords = [];
        $seenKrsKeys = [];
    ...
    private function upsertKrsDetails(array $data, \Illuminate\Support\Collection $krsIdMap, \Illuminate\Support\Collection $matkulMap, array &$errors): int
    {
        $detailRecords = [];
        foreach ($data as $item) {
    ...
    private function upsertNilaiRecords(array $data, \Illuminate\Support\Collection $mahasiswaMap, \Illuminate\Support\Collection $matkulMap, ?int $semesterId, array &$errors): int
    {
        $upsertData = [];
        foreach ($data as $item) {
            $mahasiswaId = $mahasiswaMap[$item['id_registrasi_mahasiswa']] ?? null;
    ...
    private function upsertAktivitasKuliahRecords(array $data, array &$errors): int
    {
        $records = [];
        foreach ($data as $item) {
            $records[] = [
    ...
    {
        $idMatkuls = collect($items)->pluck('id_matkul')->filter()->unique()->toArray();
        $kodeMks = collect($items)->map(fn($d) => $d['kode_mata_kuliah'] ?? $d['kode_mk'] ?? null)->filter()->unique()->toArray();
        ...
        foreach ($items as $detail) {
            ...
            $mk = $idMatkul ? $mkById->get($idMatkul) : null;
            if (!$mk && $kodeMk) {
                $mk = $mkByKode->get($kodeMk);
            }
    ...
    private function processAllSemestersBulkSync(string $syncMethod, int $offset, int $limit, ?string $syncSince): array
    {
        ...
        $currentSemester = $semesters[$semesterIndex];
        $result = $this->$syncMethod($internalOffset, $limit, $currentSemester, $syncSince);

        if ($result['has_more']) {
            $newGlobalOffset = ($semesterIndex * $OFFSET_MULTIPLIER) + $result['next_offset'];
            $hasMore = true;
        } else {
